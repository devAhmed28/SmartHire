...
{
    public class RejectOfferCommandHandler : IRequestHandler<RejectOfferCommand, Result<OfferResponse>>
    {
        ...
        public async Task<Result<OfferResponse>> Handle(RejectOfferCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                return Error.Forbidden("You do not have permission to reject this offer");
            }

            if (offer.IsAccepted)
            {
                return Error.Conflict("This offer has already been accepted and cannot be rejected");
            }
